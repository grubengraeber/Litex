...
import { db } from "@/db";
import { messages, messageReads, notifications, users, tasks } from "@/db/schema";
  ...
  try {
    ...
    const reads = messageIds.length > 0 
      ? await db
          ...
          .where(eq(messageReads.messageId, messageIds[0])) // TODO: use inArray for all
      : [];

    // Build read receipts map
    const readsByMessage: Record<string, Array<{ userId: string; userName: string | null; readAt: Date | null }>> = {};
    for (const read of reads) {
      if (!readsByMessage[read.messageId]) {
        readsByMessage[read.messageId] = [];
      }
      readsByMessage[read.messageId].push({
        userId: read.userId,
    ...
    const allReads = messageIds.length > 0 
    ...
    // Build proper reads map
    const readsMap: Record<string, Array<{ userId: string; userName: string | null; readAt: Date | null }>> = {};
...
export async function POST(
  ...
  try {
    ...
    const otherUsers = task.company?.users?.filter(u => u.id !== session.user.id) || [];
