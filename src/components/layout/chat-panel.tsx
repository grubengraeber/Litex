...
export function ChatPanel({
  ...
  if (collapsible && isCollapsed) {
    return (
      <div className="w-12 bg-card border-l border-border flex flex-col h-full">
        <button
          onClick={() => setIsCollapsed(false)}
          className="h-14 flex items-center justify-center border-b border-border hover:bg-muted transition-colors"
          title="Chat öffnen"
        >
        ...
        <div className="flex-1 flex flex-col items-center pt-4 gap-2">
          ...
          {messages.length > 0 && (
            <span className="text-xs text-muted-foreground font-medium">{messages.length}</span>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className={cn(
      ...
      hideHeader ? "w-full" : "w-80 border-l border-border"
    )}>
        ...
        <div className="h-14 px-4 flex items-center justify-between border-b border-border shrink-0">
          {collapsible && (
            <button
              ...
              className="p-1 -ml-1 mr-2 hover:bg-muted rounded transition-colors"
                      ...
                      {msg.attachments.map((att) => (
                        <button
                          ...
                          className={cn(
                            ...
                            att.status ? FILE_STATUS_STYLES[att.status] : "bg-muted border-border hover:bg-muted",
      ...
      {pendingFiles.length > 0 && (
        <div className={cn(
          ...
          <div className={cn(hideHeader && "max-w-xl mx-auto")}>
            <div className="text-xs text-muted-foreground mb-1">Anhänge ({pendingFiles.length})</div>
            <div className="flex flex-col gap-1">
              {pendingFiles.map((file, i) => (
                <div 
                  key={i}
                  className="flex items-center gap-2 px-2 py-1 bg-primary/5 rounded text-xs text-primary"
                >
